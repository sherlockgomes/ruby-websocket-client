...
class WebSocketClient < BaseClient
  ...
  DEFAULT_RETRY_DELAY = 5 # Tempo de espera entre tentativas de reconexão
  MAX_RETRY_DELAY = 15 # Tempo máximo para aguardar a próxima tentativa de reconexão
  MAX_QUEUE_SIZE = 15000 # Importante se vier um grande volume de mensagens simultâneas, como em caso de sync
  QUEUE_OVERFLOW_STRATEGY = :drop_oldest  # Remover a mensagem mais antiga da fila se a fila estiver cheia
  MAX_THREAD_WAIT_TIME = 5 # Tempo máximo para aguardar threads finalizarem
  ...
  def clear_queue
    ...
    log "Fila limpa: #{cleared_count} mensagens descartadas" if cleared_count > 0
  end
